    ...
    , m_exposedAsCover(false)
{
    ...
    if(surface) {
        connect(surface, &QWaylandSurface::surfaceDestroyed, this, [this]() {
            ...
            if (QWaylandClient *client = surface ? surface->client() : nullptr) {
                qDebug() << "Surface was destroyed" << client->processId();
            }
        ...
        connect(surface, &QWaylandSurface::hasContentChanged, this, [this](){
            ...
            if (!surface) {
                ...
                if (QWaylandClient *client = surface->client()) {
                    qDebug() << "Surface does not have content" << client->processId();
                }

                m_compositor->surfaceUnmapped(this, surface);
            } else if (!m_mapped) {
                if (QWaylandClient *client = surface->client()) {
                    qDebug() << "Surface does have content" << client->processId();
                }

                m_mapped = true;
                if (m_extSurface) {
                    m_category = m_extSurface->windowProperty(QStringLiteral("CATEGORY")).toString();
                    qDebug() << "Its category is" << m_category;
                }
...
void LipstickCompositorWindow::setExtendedSurface(QtWayland::ExtendedSurface *extSurface)
{
    ...
    updateMouseRegion(extSurface->windowProperty(QStringLiteral("MOUSE_REGION")));
}
...
QSGNode *LipstickCompositorWindow::updatePaintNode(QSGNode *old, UpdatePaintNodeData *data)
{
    ...
    const bool hwBuffer = buffer.bufferFormatEgl() == QWaylandBufferRef::BufferFormatEgl_RGB
